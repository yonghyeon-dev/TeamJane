...
export const themeUtils = {
    ...
    // 그라데이션 생성
    createGradient: (from, to, direction = "to-r") => {
        return `bg-gradient-${direction} from-[${from}] to-[${to}]`;
    },
    // 기획서 색상 가져오기
    getWeaveColor: (colorName) => {
...
export const responsiveUtils = {
    ...
    // 반응형 클래스 생성
    getResponsiveClass: (baseClass, responsive) => {
...
export const animationUtils = {
    // 애니메이션 지연 시간 계산
    getDelay: (index, baseDelay = 100) => {
        return index * baseDelay;
    },
    // 스태거 애니메이션 클래스 생성
    getStaggerClass: (index, baseDelay = 100) => {
...
export const accessibilityUtils = {
    ...
    // 키보드 네비게이션
    handleKeyboardNavigation: (event, onEnter, onEscape, onArrowUp, onArrowDown) => {
        switch (event.key) {
            case "Enter":
            ...
            case "ArrowUp":
                event.preventDefault();
                onArrowUp === null || onArrowUp === void 0 ? void 0 : onArrowUp();
                break;
            case "ArrowDown":
                event.preventDefault();
                onArrowDown === null || onArrowDown === void 0 ? void 0 : onArrowDown();
                break;
        }
    },
};
...
export const componentUtils = {
    ...
    // 반응형 클래스 생성
    responsiveClass: (baseClass, responsive) => {
        return Object.entries(responsive).reduce((acc, [breakpoint, className]) => {
            return `${acc} ${breakpoint}:${className}`;
        }, baseClass);
    },
